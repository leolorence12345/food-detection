/**
 * AWS Cognito Custom Message Lambda
 *
 * Attach this to your User Pool: User pool → User pool properties → Lambda triggers
 * → Custom message → Select this function.
 *
 * - ForgotPassword (when app calls resetPassword for delete-account) → DELETE account email
 * - SignUp / ResendCode (sign-in OTP) → keep default or return your sign-in template
 *
 * So: Verification message type = sign-in, ForgotPassword from this Lambda = delete email.
 */

const DELETE_ACCOUNT_HTML = `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Verify Account Deletion</title>
</head>
<body style="margin: 0; padding: 0; font-family: 'Segoe UI', Arial, sans-serif; background-color: #f5f7f5;">
<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #f5f7f5;">
<tr>
<td style="padding: 40px 20px;">
<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="max-width: 480px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.08);">
<tr>
<td style="padding: 40px 40px 20px 40px; text-align: center; background: linear-gradient(135deg, #7BA21B 0%, #5d7d14 100%); border-radius: 16px 16px 0 0;">
<div style="width: 80px; height: 80px; margin: 0 auto 16px auto; background-color: #ffffff; border-radius: 50%; display: flex; align-items: center; justify-content: center; padding: 12px;">
<img src="https://ukcal-assets.s3.amazonaws.com/logo.png" alt="UKcal Logo" style="width: 100%; height: 100%; object-fit: contain;" />
</div>
<h1 style="margin: 0; color: #ffffff; font-size: 28px; font-weight: 700; letter-spacing: -0.5px;">UKcal</h1>
<p style="margin: 8px 0 0 0; color: rgba(255,255,255,0.9); font-size: 14px;">Smart Nutrition Tracking</p>
</td>
</tr>
<tr>
<td style="padding: 40px;">
<h2 style="margin: 0 0 16px 0; color: #1f2937; font-size: 22px; font-weight: 600; text-align: center;">Verify Account Deletion</h2>
<p style="margin: 0 0 8px 0; color: #1f2937; font-size: 15px; line-height: 1.6; text-align: center;">Hello, {username}.</p>
<p style="margin: 0 0 30px 0; color: #6b7280; font-size: 15px; line-height: 1.6; text-align: center;">
Enter this code in the app to confirm you want to delete your account. This code will expire in 5 minutes.
</p>
<div style="background: linear-gradient(135deg, #f0f7e6 0%, #e8f5d6 100%); border: 2px solid #7BA21B; border-radius: 12px; padding: 24px; text-align: center; margin-bottom: 30px;">
<p style="margin: 0 0 8px 0; color: #5d7d14; font-size: 12px; font-weight: 600; text-transform: uppercase; letter-spacing: 1px;">Your Verification Code</p>
<p style="margin: 0; color: #7BA21B; font-size: 36px; font-weight: 700; letter-spacing: 8px; font-family: 'Courier New', monospace;">{####}</p>
</div>
<p style="margin: 0 0 16px 0; color: #9ca3af; font-size: 13px; text-align: center; line-height: 1.5;">
If you didn't request account deletion, you can safely ignore this email.
</p>
<p style="margin: 0; color: #6b7280; font-size: 13px; text-align: center; line-height: 1.5;">
After you confirm, your account will be deleted. This action cannot be undone.
</p>
</td>
</tr>
<tr>
<td style="padding: 24px 40px; background-color: #f9fafb; border-radius: 0 0 16px 16px; border-top: 1px solid #e5e7eb;">
<p style="margin: 0 0 8px 0; color: #6b7280; font-size: 12px; text-align: center;">© 2025 UKcal. All rights reserved.</p>
<p style="margin: 0; color: #9ca3af; font-size: 11px; text-align: center;">This is an automated message. Please do not reply.</p>
</td>
</tr>
</table>
</td>
</tr>
</table>
</body>
</html>`;

exports.handler = async (event) => {
  const { userName } = event;

  if (event.triggerSource === 'CustomMessage_ForgotPassword') {
    // Delete-account OTP: app called resetPassword → send delete-account email (same as Invitation template)
    // Keep {####} in the message; Cognito replaces it with the verification code before sending
    event.response.emailSubject = 'Verify account deletion – UKcal';
    event.response.emailMessage = DELETE_ACCOUNT_HTML.replace(/\{username\}/g, userName);
    return event;
  }

  // Sign-in / verification: CustomMessage_SignUp, CustomMessage_ResendCode, etc.
  // Return event unchanged so Cognito uses your default Verification message from the console
  return event;
};
